...
import { type IResponce, type GetAll,IRestrictions } from "#types/api/common";
...
class UserApi {
    ...
    public async registration(data:UserRegistration):Promise<IResponce>{
        return (await host.post(this.route + "registration", data)).data;
    }
    public async getAll({limit = 10,offset = 0}:IRestrictions):Promise<GetAll<IUser>>{
